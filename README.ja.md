...
Claude Code に没頭しているとき、つい雑で冗長なプロンプトを打ってしまいます：

> users のクエリ、行数が多いとなんか遅い気がする、見てもらえる？たぶんインデックスのせいかも、よく分かんないけど

プロンプトエンジニアリングが推奨する書き方 —— 明確・直接的・曖昧さなし —— にすると、
**求めている内容を変えずに**、より良く動きます：

> `users` クエリが大量の行で遅くなる原因を調査し、インデックスが原因かどうか確認して。

同じ依頼、同じ範囲 —— ただ明確に表現しただけ。普段なら手を止め、別のチャットを開き、貼り付け、
「綺麗なプロンプトに書き換えて」と頼み、待って、コピーして戻し、ようやく実行する。**毎回これです。**

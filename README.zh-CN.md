...
你正沉浸在 Claude Code 里，随手敲下一句啰嗦的 prompt：

> users 查询在行数多的时候好像有点慢，能看下不，可能是索引问题吧不太确定

如果按 prompt engineering 推荐的方式来写 —— 清晰、直接、无歧义 —— 效果会更好，
而且**不改变你要表达的意思**：

> 排查 `users` 查询在大数据量下为何变慢，并检查是否是索引导致的。

同样的需求、同样的范围 —— 只是表达得更清楚。平时你得停下来，另开一个对话，粘贴进去，
问一句"帮我改成清晰的 prompt"，等结果，再复制回来，最后才运行。**每一次都这样。**

Prompet 把这一整段绕路全省了。它就住在你的会话里，在你主动触发的那一刻替你改写好 prompt。

## 工作原理

Prompet 注册了一个 Claude Code 的 **`UserPromptSubmit` hook**。当你提交一条被标记为需要优化的
...
- **只改写，绝不加料。** 它把你的措辞改写得更符合 prompt engineering 规范 —— 清晰、具体、无歧义 ——
  ...
  用对名字，**绝不会**新增任何需求。
- **按需触发，不碍事。** Prompet 只处理你标记的 prompt，其余的原样直达。想让它对每条都生效，
  用 `prompet mode auto`。
- **设计上很安全。** Claude 仍然能看到你的原始 prompt；改写版是作为引导*追加*进去的，而非悄悄替换。
  一旦出错或超时，你的原始 prompt 会**原封不动**地通过 —— Prompet 永远不会卡住你。
...
装好后，在任意 prompt 前加上 `pp ` 前缀，Prompet 就会在 Claude 运行前替你改写：

```text
pp users 查询有点慢，看一下
```

Prompet 会把它改写成干净、符合 prompt engineering 规范的版本（意思不变）再交给 Claude。
**没加** marker 的 prompt 会被完全放过 —— Prompet 只在你主动触发时才动手。
（想对每条都生效？`prompet mode auto`。）
...
| `manual` | 从不自动改写；需要时用 `/prompet:optimize`。 |
...
```bash
prompet set optimizeModel claude-haiku-4-5   # 更快更省地改写
prompet mode auto                            # 对每条实质性 prompt 都改写（手动开启）
prompet lang zh                              # 强制界面语言
...
prompet lang <code>                      界面语言（auto | en | zh | ja）
prompet config | set <键> <值>           配置
prompet optimize [文本]                  改写一段 prompt 并打印
